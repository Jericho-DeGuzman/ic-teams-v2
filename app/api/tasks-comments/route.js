...
export async function GET(req) {
    ...
    try {
        const response = await microservice(`/ic-teams/task-comments?task_uuid=${uuid}`)
...
export async function POST(req) {
    ...
    try {
        const response = await microservice.post('/ic-teams/task-comments', newComment );
        return NextResponse.json({ status: 200, data: response.data });
    } catch (error) {
        console.log(error)
        return NextResponse.json({ status: error?.response.status, message: error?.response.data });
