...
export default function SelectCategories({ placeholder, label, onchange, disabled, defaultValue }) {
    ...
    return (
        <div className='w-full space-y-1'>
            ...
            <Select placeholder={placeholder} isLoading={loading} options={selection} isSearchable={true} onChange={onchange} 
                isDisabled={disabled} value={selection.filter(val => val.value == defaultValue)}/>
        </div>
    )
}